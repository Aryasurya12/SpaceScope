...
import StargazingFinder from './StargazingFinder';
...
const Icons: Record<string, () => React.JSX.Element> = {
    ...
    Radio: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>,
    Telescope: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" /></svg>,
                ...
                <div className="flex-1 p-6 overflow-y-auto relative z-20 custom-scrollbar">
                    <AnimatePresence mode="wait">
                        <motion.div key={section} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="h-full">
                            ...
                            {section === DashboardSection.WEATHER && (
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        {[
                                            { label: 'Radio Blackout', key: 'r', color: 'text-cyan-400', border: 'border-cyan-500/20', shadow: 'shadow-cyan-500/10' },
                                            { label: 'Geomagnetic Scale', key: 'g', color: 'text-yellow-400', border: 'border-yellow-500/20', shadow: 'shadow-yellow-500/10' },
                                            { label: 'Solar Radiation', key: 's', color: 'text-purple-400', border: 'border-purple-500/20', shadow: 'shadow-purple-500/10' }
                                        ].map((item) => (
                                            <motion.div
                                                key={item.key}
                                                ...
                                                onClick={() => window.dispatchEvent(new CustomEvent('ai-query', { detail: `Explain ${item.label} and how level ${displaySolar?.[0]?.[item.key]?.value || 0} affects spacecraft.` }))}
                                            >
                                                ...
                                                <div className={`text-6xl font-display font-black ${item.color} mb-4 flex items-baseline gap-2`}>
                                                    {displaySolar?.[0]?.[item.key]?.value ?? '0'}
                                                    <span className="text-xs font-mono text-gray-500 font-normal">/ 5</span>
                                                </div>
                                                <div className="font-mono text-[10px] tracking-widest uppercase py-1 px-3 bg-white/5 rounded-full inline-block mb-4">
                                                    {displaySolar?.[0]?.[item.key]?.text || 'Quiet'}
                                                </div>
                                                <div className="pt-4 border-t border-white/5 flex justify-between items-center text-[9px] text-gray-500 uppercase tracking-widest font-bold">
                                                    <span>24h Peak</span>
                                                    <span className={displaySolar?.[1]?.[item.key]?.value > 0 ? 'text-amber-400' : ''}>
                                                        Level {displaySolar?.[1]?.[item.key]?.value || '0'}
                                                    </span>
                                                </div>
                                            </motion.div>
                                        ))}
                            ...
                            {section === DashboardSection.NEWS && <NewsHub />}
                            {section === DashboardSection.STARGAZING && <StargazingFinder />}
